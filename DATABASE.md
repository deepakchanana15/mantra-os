...
Prisma schema: [`packages/db/prisma/schema.prisma`](packages/db/prisma/schema.prisma) — 48 models across 11 domains, validated with `npx prisma validate`.
...
**Deliberately no RLS policy on:** `organizations` and `users` (they aren't scoped *by* organizationId — a User belongs to many orgs, and Organization is the tenant boundary itself, not a child of one; access is governed by the app-layer Membership check instead), `roles`/`permissions`/`role_permissions` (shared reference data for V1's fixed system roles — revisit once V2 adds per-org custom roles), `user_preferences` (scoped by `userId`, protected by the API checking `req.user.id`, not by tenant), `currencies` (global ISO 4217 reference data, same reasoning as roles/permissions), and `lead_intake_keys` (looked up by a public, unauthenticated endpoint with no org context to set yet — the key itself is the identifier; protected by being an unguessable secret and by the admin-facing repository explicitly filtering every query by `organizationId` itself, see [DECISIONS.md](DECISIONS.md) "Public lead intake: website forms, Phase 5 (lead attribution)"). `companies`/`countries`/`brands`/`websites` (added Phase 8) get the standard `organizationId`-based policy like every other tenant-scoped table.
