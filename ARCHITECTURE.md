...
| **Sales** | Quote, QuoteLine, SalesOrder, SalesOrderLine, Shipment, ShipmentLine, Opportunity, Invoice, InvoiceLine | Opportunity/Invoice added Sub-phase C (see [DECISIONS.md](DECISIONS.md)). Quote gained an optional `opportunityId` FK, and Invoice gained optional itemized `InvoiceLine`s (server-computed total when present) — see DECISIONS.md "Feature batch: Product currency, Opportunity→Quote link, Invoice lines, SupportTicket SLA, Member creation". SalesOrder gained a required-going-forward `salesChannel` (Online/Offline) with conditional sub-fields — see DECISIONS.md "Feature batch: multi-document attachments, Supplier phones, Sales Channel". |
