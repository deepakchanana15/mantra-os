...
| **Global** (added Phase 8) | Company, Country, Brand, Website, Currency | `Organization → Company → Country → Warehouse`, with Brand/Website cross-cutting. See [DECISIONS.md](DECISIONS.md) "Global multi-country, multi-company, multi-brand architecture". Sub-phase B scoped Customer/Quote/SalesOrder/PurchaseOrder/Supplier to Company/Country and Product/Campaign to Brand (all nullable). |
| **CRM** | Customer, Contact | |
| **Products** | Product, Category (self-referencing hierarchy) | Product's display currency follows `country.currency` → `company.baseCurrency` → USD fallback, resolved client-side — see [DECISIONS.md](DECISIONS.md) "Product currency follows Company/Country". |
...
| **Sales** | Quote, QuoteLine, SalesOrder, SalesOrderLine, Shipment, ShipmentLine, Opportunity, OpportunityStageHistory, Invoice, InvoiceLine, LeadIntakeKey, WhatsAppPhoneNumber, WhatsAppMessage | Opportunity/Invoice added Sub-phase C (see [DECISIONS.md](DECISIONS.md)). Quote gained an optional `opportunityId` FK, and Invoice gained optional itemized `InvoiceLine`s (server-computed total when present) — see DECISIONS.md "Feature batch: Product currency, Opportunity→Quote link, Invoice lines, SupportTicket SLA, Member creation". SalesOrder gained a required-going-forward `salesChannel` (Online/Offline) with conditional sub-fields — see DECISIONS.md "Feature batch: multi-document attachments, Supplier phones, Sales Channel". Opportunity gained `source`/UTM/`externalLeadId` attribution fields, and a public `POST /v1/leads/intake` route (auth'd by a per-Company `LeadIntakeKey`, not a session) turns a website form submission into an Opportunity automatically — see DECISIONS.md "Public lead intake: website forms, Phase 5 (lead attribution)". A second public route, `POST /v1/whatsapp/webhook` (HMAC-signature-verified, org resolved via `WhatsAppPhoneNumber`), does the same for incoming WhatsApp messages — grouped into one Opportunity per phone number until Won/Lost, logged to `WhatsAppMessage` — see DECISIONS.md "WhatsApp lead capture". **`Opportunity.customerId` is nullable**: a lead with no matching Customer carries its own `leadName`/`leadEmail`/`leadPhone` until a human explicitly calls `POST /v1/opportunities/:id/convert-to-customer` — see DECISIONS.md "Leads are not Customers". Customer and Opportunity both carry a nullable, human-readable `code` (e.g. `MS-AU-20260726-01`, generated by the shared `BusinessCodeService` in Common) — see DECISIONS.md "Business ID codes for Customers and Opportunities". `OpportunityStageHistory` tracks time spent in each pipeline stage — see DECISIONS.md "Opportunity stage-duration tracking". |
| **Purchasing** | Supplier, PurchaseOrder, PurchaseOrderLine, GoodsReceipt, Expense | GoodsReceipt feeds Inventory. Expense is soft-deletable, unlike append-only GoodsReceipt — optionally linked to a GoodsReceipt/PurchaseOrder/Supplier (Supplier is optional — petty cash, parking, courier expenses have none). Both GoodsReceipt and Expense support multiple attached documents (supplier invoice, delivery challan, packing slip, GRN copy, receipts) via the shared polymorphic `Attachment` table, uploaded direct to Vercel Blob from the browser as private-access blobs — viewing one goes through `/api/attachments/view-url` (issues a short-lived signed URL) rather than a plain link, see [DECISIONS.md](DECISIONS.md) "Attachments switched to private access". Supplier gained a `SupplierPhone` model (multiple numbers, one primary) alongside its existing full-address Json field. |
...
| **Settings** | OrganizationSettings, UserPreferences | Deliberately thin. `OrganizationSettings.codePrefix` (default `"ORG"`, `"MS"` for the real Mantra Sports org) is the brand prefix `BusinessCodeService` uses for Customer/Opportunity business IDs — see DECISIONS.md "Business ID codes for Customers and Opportunities". |
| **Marketing** | Segment, EmailTemplate, Campaign, MarketingIntegration, AdCampaignMetric, AdCampaign | Email-only for V1 — a Campaign sends an EmailTemplate to a saved Segment (a filter over CRM Customers/Contacts) via Brevo, with aggregate delivery/open/click tracking via Brevo webhooks (see [DECISIONS.md](DECISIONS.md) "Switched email provider to Brevo"). Has a real frontend (Campaigns/Segments/Templates tabs) — not a gap. Ad-platform integrations (Meta live, Google/Bing planned) pull campaign spend/performance via a daily Vercel Cron sync into `AdCampaignMetric` (day-granular). **`AdCampaign`** is a separate, permanent per-campaign archive (name, status, live-since date, lifetime + rolling last-30-day totals, sourced from Meta's own date presets, never deleted) — the "Ad Campaigns" tab on the Marketing page shows every campaign ever run with no filtering, while the Dashboard/Reports widget shows only campaigns with last-30-day activity — see DECISIONS.md "Per-campaign ad performance, not channel-consolidated". No landing pages — those would need vendors outside the current stack. |
...
The Owner/Admin-only "Users & Roles" bundle now includes `members:create` — an Owner or Admin can create a teammate's login directly from Settings (email/name/temporary password/role), rather than only editing existing members' roles. No invite email; see [DECISIONS.md](DECISIONS.md) "Member creation, no self-service invite".
