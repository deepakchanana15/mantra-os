...
| **Sales** | Quote, QuoteLine, SalesOrder, SalesOrderLine, Shipment, ShipmentLine, Opportunity, Invoice, InvoiceLine, LeadIntakeKey | Opportunity/Invoice added Sub-phase C (see [DECISIONS.md](DECISIONS.md)). Quote gained an optional `opportunityId` FK, and Invoice gained optional itemized `InvoiceLine`s (server-computed total when present) — see DECISIONS.md "Feature batch: Product currency, Opportunity→Quote link, Invoice lines, SupportTicket SLA, Member creation". SalesOrder gained a required-going-forward `salesChannel` (Online/Offline) with conditional sub-fields — see DECISIONS.md "Feature batch: multi-document attachments, Supplier phones, Sales Channel". Opportunity gained `source`/UTM/`externalLeadId` attribution fields, and a public `POST /v1/leads/intake` route (auth'd by a per-Company `LeadIntakeKey`, not a session) turns a website form submission into an Opportunity automatically — see DECISIONS.md "Public lead intake: website forms, Phase 5 (lead attribution)". **`Opportunity.customerId` is nullable**: a lead with no matching Customer carries its own `leadName`/`leadEmail`/`leadPhone` until a human explicitly calls `POST /v1/opportunities/:id/convert-to-customer` — see DECISIONS.md "Leads are not Customers". Customer and Opportunity both carry a nullable, human-readable `code` (e.g. `MS-AU-20260726-01`, generated by the shared `BusinessCodeService` in Common) — see DECISIONS.md "Business ID codes for Customers and Opportunities". |
