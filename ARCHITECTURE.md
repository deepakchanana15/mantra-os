...
| **Sales** | Quote, QuoteLine, SalesOrder, SalesOrderLine, Shipment, ShipmentLine, Opportunity, Invoice, InvoiceLine, LeadIntakeKey | Opportunity/Invoice added Sub-phase C (see [DECISIONS.md](DECISIONS.md)). Quote gained an optional `opportunityId` FK, and Invoice gained optional itemized `InvoiceLine`s (server-computed total when present) — see DECISIONS.md "Feature batch: Product currency, Opportunity→Quote link, Invoice lines, SupportTicket SLA, Member creation". SalesOrder gained a required-going-forward `salesChannel` (Online/Offline) with conditional sub-fields — see DECISIONS.md "Feature batch: multi-document attachments, Supplier phones, Sales Channel". Opportunity gained `source`/UTM/`externalLeadId` attribution fields, and a public `POST /v1/leads/intake` route (auth'd by a per-Company `LeadIntakeKey`, not a session) turns a website form submission into a Customer + Opportunity automatically — see DECISIONS.md "Public lead intake: website forms, Phase 5 (lead attribution)". |
