...
| **Sales** | Quote, QuoteLine, SalesOrder, SalesOrderLine, Shipment, ShipmentLine, Opportunity, Invoice | Opportunity/Invoice added Sub-phase C (see [DECISIONS.md](DECISIONS.md)) — minimal versions, Opportunity not yet linked to Quote, Invoice not itemized. SalesOrder gained a required-going-forward `salesChannel` (Online/Offline) with conditional sub-fields — see DECISIONS.md "Feature batch: multi-document attachments, Supplier phones, Sales Channel". |
