...
**Incident: this deploy took the entire prod API down, not just WhatsApp, for a few minutes.** `WhatsAppWebhookController`'s constructor used `config.getOrThrow()` for `WHATSAPP_VERIFY_TOKEN`/`WHATSAPP_APP_SECRET`, which only existed in dev's `.env` — Vercel's prod env vars hadn't been set yet. NestJS instantiates every controller at application bootstrap, so one missing config value for one not-yet-configured feature crashed the whole app (every route returned 500, `FUNCTION_INVOCATION_FAILED`) until a hotfix landed — `getOrThrow` → `config.get() ?? ""`, so a missing value now just makes WhatsApp requests fail their own verification (401) instead of preventing the process from starting at all. **Lesson, now a standing rule**: never use `getOrThrow` for a env var in a controller/service constructor unless that env var is required for the *entire application* to function — for anything feature-scoped, read it lazily (inside the method that needs it) or fall back to a safe "this feature is unavailable" default, so an unconfigured optional integration can never become a total outage.
...
**Bug found the same day, before this had even synced against a real account:** every spend figure across the Dashboard widget and the Ad Campaigns tab was formatted with a hardcoded `Intl.NumberFormat(..., { currency: "USD" })`. The user's real AU ad account bills in INR — confirmed against a live Ads Manager screenshot showing ₹1,271.19 spent — so the number itself was always right, only the currency symbol was wrong (and would be wrong for any non-USD account). The frontend's `formatMoney()` helper now falls back to a **plain, unlabeled number** (not USD) when a campaign's currency isn't known — silently defaulting to USD a second time would repeat the exact bug for the next non-USD account, so "no symbol" is the safer failure mode than "wrong symbol."

**First fix attempt (a separate `/act_<id>?fields=currency` object fetch) silently returned nothing:** confirmed via a real prod sync — the daily metrics and full campaign archive both populated correctly (matching the Ads Manager screenshot exactly: same 3 campaigns, same spend figures), but `AdCampaign.currency` stayed null on every row with no error recorded anywhere, meaning that specific call returned 200 with no `currency` field rather than throwing. Rather than debug a call this environment can't directly test against the live API, switched to the more reliable path: Meta's Insights API accepts `account_currency` as a request-able field on the exact same `/insights` edge the (already-working) lifetime/last-30-day calls use — one value, same for every row on a given account, with zero extra API calls or new permission surface.
...
**Schema — two new tables, shared across every future ad channel:** `MarketingIntegration` (one row per org+channel: `accessToken`, `accountId`, `status`, `lastSyncedAt`, `lastError`) stores the credential; `AdCampaignMetric` (one row per external campaign per day: `impressions`, `clicks`, `spend`, `leads`) is the shared shape every channel's sync job writes into, upserted on `(organizationId, channel, externalCampaignId, date)` so re-running a sync is idempotent. `AdCampaignMetric` deliberately has no `createdBy`/`updatedBy` — it's machine-written by a scheduled job, not a user action, same reasoning as `StockLevel` being a derived/system-maintained snapshot (see DATABASE.md).

**Credential access is Owner/Admin-only, even for reads** — `marketing_integrations` joins `org_settings`/`members` in `rolesFor()` as a resource restricted for every action, since it stores a real access token per channel. `ad_campaign_metrics` (the pulled numbers themselves) are readable by everyone, same as `reports`/`inventory` reads — the credential is sensitive, the resulting spend/impression counts aren't.

**Meta token setup deliberately skips Meta's public App Review** — since this is MantraOS pulling data for its own ad account, not a public app requesting access to other businesses' accounts, a long-lived System User access token (generated directly in Meta Business Manager, scoped to `ads_read` on the one ad account) is sufficient. No OAuth consent flow, no waiting on Meta's review queue — this is the reason Meta was sequenced before Google/Bing, which do require an external developer-token approval process.

**Daily sync via Vercel Cron, not real-time** — matches the "100% Vercel, no persistent background worker" constraint locked in Phase 1 hosting decisions. `GET /v1/internal/sync-ad-metrics` (Vercel Cron's own convention: setting a `CRON_SECRET` env var makes Vercel send `Authorization: Bearer <CRON_SECRET>` automatically) iterates every organization, opens one RLS-scoped transaction per org (same mechanism `TenantContextInterceptor` uses per-request, just manually driven by a loop over every org id instead of a single request's header — `organizations` itself carries no RLS policy, so listing every org id first is safe), and syncs each connected channel. A manual "Sync now" button in the Integrations tab hits the same underlying service method for on-demand syncs between cron runs.

**Bug found and fixed during verification — sync-failure recording was silently rolled back:** the original `sync()` implementation recorded a failure (`status: ERROR`, `lastError`) and then threw a `BadRequestException` so the caller sees the failure — but both happened inside the same request-scoped Prisma transaction (`TenantContextInterceptor`'s), and a transaction whose callback throws rolls back everything written during it, including the failure record itself. Fixed by writing the failure through its own independent transaction (a fresh `prisma.$transaction` with its own `SET LOCAL app.current_org_id`, not the request's `tx`) before throwing, so the failure genuinely persists regardless of what happens after. Caught by a dedicated smoke test asserting the `ERROR`/`lastError` fields actually landed, not just that the HTTP call returned 400.

**A real, self-inflicted incident during this work, worth recording:** generating the migration diff script for these two tables used `prisma migrate diff --shadow-database-url <dev-url>` pointed at the *same* live dev database instead of a genuinely separate shadow database. Prisma's shadow-database workflow resets its target to a clean state and replays migration history into it — pointed at the real dev database, this wiped all dev data (organizations, users, RBAC catalog, global master data) down to an empty-but-correctly-migrated schema, and separately blew away the `mantraos_app` role's schema grants (a `setup-app-role.js` re-run restores those, but that script also rotates the role's password and rewrites `apps/api/.env` — expected and fine for dev, never acceptable to run against prod). Production was never touched — it uses a completely separate Neon project/hostname, and this operation only ever pointed at dev's connection string. Recovered by baselining the 9 pre-existing migrations as applied (`prisma migrate resolve --applied <name>` per migration), deploying the new one normally, then re-running `seed-rbac.js`, `create-demo-user.js`, and `seed-global-domain.js` to restore dev to a working state. **Lesson: `--shadow-database-url` must always point at a genuinely disposable database, never at the real dev connection string** — Prisma has no confirmation prompt before resetting whatever it's pointed at.

**Reversibility:** High for the schema/RBAC/UI. The Meta-specific pieces (`MetaAdsService`'s Graph API shape, the event/field names it requests) are the one genuinely vendor-specific part — swapping in Google or Bing later means a new service file with the same `fetchCampaignInsights`-shaped method, not a rework of `MarketingIntegration`/`AdCampaignMetric` or the Dashboard widget.
...
**Why reversed:** The original reasoning ("the app is already authenticated end-to-end, and the URL's random suffix makes it unguessable") undersold one thing: a public blob's URL requires **no authentication at all** once someone has it — MantraOS's login only gates the *page* that shows the link, not the file itself. Walking through realistic ways a link could leave the org (forwarded in an email/WhatsApp message to a vendor or accountant, sitting in browser history on a shared/borrowed device, exposed via a lost/compromised device, outliving an ex-employee's access, or leaking in a future database breach), the user judged that risk unacceptable even though the documents involved (receipts, vendor invoices) aren't highly sensitive — consistent with [[feedback_destructive_actions]]-style caution about anything hard to fully undo. A leaked public link never expires and needs no further credential; a leaked private one is already a dead end.

**What changed:**
- `MultiFileUpload`'s `upload()` call now requests `access: "private"` instead of `"public"` — this alone was necessary just to make uploads succeed at all against this store, independent of the security reasoning above.
- New `apps/web/app/api/attachments/view-url` route: given a stored blob URL, issues a signed token (`issueSignedToken`) scoped to that exact pathname with a 5-minute expiry, then exchanges it for a real fetchable URL (`presignUrl`) — no file bytes proxy through the Next.js server, only a short-lived link is generated. Gated by the same "logged in with an org selected" baseline as every other Next.js route (`/api/uploads` included) — the real authorization already happened when the attachment's parent record was created and returned to that org.
- New shared `AttachmentLink` client component replaces every plain `<a href={attachment.url}>` across the app (Goods Receipt detail, Expense list, and `MultiFileUpload`'s own upload preview) with a button that fetches a signed URL on click, then opens it — since a stored URL is no longer directly clickable on its own.
- Verified end-to-end with a real private blob: the raw URL is confirmed non-fetchable without signing, the new route produces a signed URL that serves the actual file content, and the route correctly refuses an unauthenticated request.

**Reversibility:** High. Nothing about the private-blob architecture is store-specific or hard to undo — reverting to public would mean deleting the signing route/component and switching the one `access` field back, though there'd be no reason to now that the signed-URL flow exists and works.
...
**Access level — `public`, not `private`:** ~~Vercel Blob's `private` access requires generating a signed URL every time a file is displayed; `public` blobs are reachable by anyone with the exact URL, but the URL includes a random suffix (`addRandomSuffix: true`) making it unguessable. Given the app is already authenticated end-to-end and this isn't public-facing content, `public` was chosen as the minimal option — upgradeable to `private` + signed URLs later if a stricter requirement emerges.~~ **Superseded 2026-07-23 — see "Attachments switched to private access" above.** The store turned out to be configured private regardless, and on reflection a public blob's URL needs no login at all once it exists outside the app, which changed the call.
