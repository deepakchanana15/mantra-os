...
**What happened:** After the prod Neon database was provisioned and `apps/api` was deployed to Vercel, every request hung for the full function duration and returned `FUNCTION_INVOCATION_TIMEOUT` rather than a real response. Several real, separate issues were found and fixed in sequence before the actual root cause was reached:
...
2. **Bootstrap failures were silent.** `api/index.ts`'s handler didn't catch a `NestFactory.create()`/`app.init()` rejection, so a genuine config error (briefly, a missing `RESEND_API_KEY` before it was correctly added to Vercel) looked identical to a hang — both ended in the same timeout. Fixed by catching and caching the failure, returning an immediate 500 with the real error message instead of retrying (and re-hanging on) the same failing bootstrap every request. This fix is still correct and stays regardless of the root cause below.
3. **Region mismatch, a reasonable but insufficient first fix.** Vercel had deployed the function to Mumbai (`bom1`) against a Neon database in `us-east-1`. Pinned `apps/api/vercel.json`'s `"regions": ["iad1"]` to match — necessary for latency, but deploying and testing again showed the hang persisted, proving cross-region distance wasn't the (sole) cause.
4. **A wrong diagnosis: IPv6, "fixed" with `dns.setDefaultResultOrder`.** Neon's pooler hostname resolves to both AAAA (IPv6) and A (IPv4) records, and Vercel's Node.js serverless runtime doesn't reliably support outbound IPv6 — a very plausible cause of a silent hang instead of a clean failure. `dns.setDefaultResultOrder("ipv4first")` was added at the top of `api/index.ts` — and made no difference. Root cause: Prisma's query engine is a native binary with its own Rust-based networking stack; it never goes through Node's `dns` module at all, so this had zero effect on Prisma's actual connection attempts. Removed once the real fix (below) was in place — leaving dead, misleading code around wasn't worth it just to document a dead end.
5. **The actual root cause and fix: switch `PrismaService` to Neon's serverless driver adapter (`@prisma/adapter-neon` + `@neondatabase/serverless`, WebSocket-based) instead of a raw Postgres TCP connection.** This is Neon's own documented, supported solution for exactly this class of problem — serverless/edge runtimes with unreliable raw TCP/IPv6 egress — not a workaround. Verified two ways before trusting it: (a) the same prod connection string, run locally through the adapter, connected in ~2s; (b) critically, a full re-run of the RLS `SET LOCAL app.current_org_id` transaction pattern (`TenantContextInterceptor`'s exact mechanism) against the real prod database through the adapter, confirming tenant isolation still holds — this touches the single most safety-critical code path in the system, so it was not shipped on faith that "the adapter probably behaves the same."

**Why this took several iterations rather than one diagnosis:** each symptom (build failure, then a hang, then still a hang after two plausible-looking fixes) looked identical from the outside — a failed or stuck deployment — and only differed in their actual causes once Vercel's Function Logs and a direct connection test (bypassing Vercel entirely, from a normal machine) were used to isolate where in the request lifecycle things actually broke. The direct test — the same pooled connection string, run locally, succeeding in ~3 seconds — was the key piece of evidence that ruled out the connection string/credentials/Neon-availability entirely and pointed at the serverless network environment specifically. The IPv6 DNS theory was directionally right (Neon's dual-stack DNS really is the trigger) but wrong about which layer to fix it at — worth remembering next time a "the DNS/network layer must be the problem" theory shows up: confirm which component actually owns the socket before patching Node's global DNS behavior.

**Reversibility:** The public/ folder, fail-fast bootstrap, and region pin are all small, low-risk, and stay regardless. The driver adapter change is slightly more involved (a different connection mechanism for one central service) but is a drop-in replacement verified against the real database and the real RLS mechanism before shipping — not a speculative change.
